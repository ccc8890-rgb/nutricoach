-- ============================================================
-- NUTRICOACH - SQL COMPLETO (tablas faltantes + correcciones)
-- ============================================================
-- Este script crea todas las tablas que faltan en la BD.
...
drop policy if exists "Coach can manage cuestionarios" on public.cuestionarios;
create policy "Coach can manage cuestionarios" on public.cuestionarios
  for all using (coach_id = auth.uid());

drop policy if exists "Anyone can read active cuestionarios by codigo" on public.cuestionarios;
...
-- ============================================================
create table if not exists public.respuestas_clientes (
  id uuid default uuid_generate_v4() primary key,
...
drop policy if exists "Coach can manage respuestas_clientes" on public.respuestas_clientes;
create policy "Coach can manage respuestas_clientes" on public.respuestas_clientes
  for all using (coach_id = auth.uid());

drop policy if exists "Anyone can insert respuestas_clientes" on public.respuestas_clientes;
